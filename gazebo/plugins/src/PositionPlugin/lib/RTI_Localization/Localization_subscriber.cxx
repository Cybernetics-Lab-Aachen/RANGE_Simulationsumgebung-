...
callback_function handlerPointer;
...
void de_dlr_enex_datamodel_range_LocalizationListener::on_data_available(DDSDataReader* reader)
{
    ...
    for (i = 0; i < data_seq.length(); ++i) {
        if (info_seq[i].valid_data) {
           ...
           // de_dlr_enex_datamodel_range_LocalizationTypeSupport::print_data(&data_seq[i]);
            de_dlr_enex_datamodel_range_Localization *sample = &data_seq[i];
            
            //Passes the recieved datastructure to our handler function
            handlerPointer(sample);
        }
    }
...
extern "C" int subscriber_main(int domainId, char* topicName) 
{
...
/*
 * creates a new subscriber on Domain domainId, topic topicName 
 * and uses callbackPointer as handler function, which is called after recieving a new data instance 
 */
extern "C" int localizationSubscriber_init(int domainId, char* topicName, callback_function callbackPointer)
{
    	
	handlerPointer=callbackPointer;
    ...
    */
    ...
    return subscriber_main(domainId, topicName);
}
